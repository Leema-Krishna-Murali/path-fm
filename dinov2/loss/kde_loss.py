...
class KDELoss(nn.Module):
    ...
    def vmF(x, y, kappa):
        ...
        sim = xnorm @ y_norm.T#Doesn't follow the formula technically, but shouldn't matter here.

        kernel = torch.exp(kappa * sim)
    
    def forward(self, student_output, eps=1e-8):
        
        kappa = 20.0#Not sure what others use, it's not listed.

        #Batch size
        n = student_output.shape[0]
        k = vmF(student_output, student_output, kappa)
