...
# Configuration
# Save inside the repository's logs directory
OUTPUT_DIR="$PWD/logs"
...
is_text_file() {
    ...
    case "$file" in
        ...
        *[cC][aA][cC][hH][eE]*/*|*/uv.lock|uv.lock|LICENSE|*LICENSE*|*.hex|\
        *.md|*build_lit*.py)
            return 1 ;;
        
        # Then check known text file types
        *.py|*.js|*.jsx|*.ts|*.tsx|*.rb|*.php|*.java|*.go|*.lock*|\
        *.c|*.cpp|*.h|*.hpp|*.swift|*.m|*.cs|*.sh|*.pl|\
        *.html|*.css|*.scss|*.less|\
        *.json|*.yaml|*.yml|*.toml|*.ini|*.env|\
        *.txt|*.rst|*.xml|*.sql|*.conf|\
...
is_excluded_dir() {
    local dir="$1"
    # Exclude any hidden directories (name starts with a dot) anywhere in path
    if [[ "$dir" == ./.?* || "$dir" == */.?* ]]; then
        return 0
    fi

    # Exclude any directory path containing 'cache' (case-insensitive)
    local dir_lc
    dir_lc=$(printf "%s" "$dir" | tr '[:upper:]' '[:lower:]')
    if [[ "$dir_lc" == *cache* ]]; then
        return 0
    fi

    case "$dir" in
        *.git|*.git/*|*node_modules|*node_modules/*|*venv|*venv/*|*__pycache__|*__pycache__/*|\
        *dist|*dist/*|*build|*build/*|*.idea|*.idea/*|*.vscode|*.vscode/*|*.claude|*.claude/*|\
        *logs|*logs/*|*wandb|*wandb/*|*.tcga_local_stats|*.tcga_local_stats/*|*conversion_logs|*conversion_logs/*|*egg-info*|\
        *output_*|*output_*/**)
            return 0 ;;
        *) return 1 ;;
    esac
...
find . -type f | sort | while read -r file; do
    # Skip excluded directories and avoid self-appending the output file
