...
The current `run_short_1gpu.sh` uses `dinov2/configs/train/vitg14_reg4_short1.yaml`, which is the same as the full reproduction's YAML config but with batch size lowered from 48 to 44 (to evade CUDO OOM), epochs lowered from 200 to 15, warmup_epochs lowered from 10 to 7, and no early stopping. On a single H100 GPU, this run completes in ~5 hours (will likely run out of memory on older GPUs).
